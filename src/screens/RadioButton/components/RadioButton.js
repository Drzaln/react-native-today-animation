...
const RadioButton = ({title, color = '#8ecae6'}) => {
  ...
  const colors = ['#ced4da', color];

  const animatedContainer = useAnimatedStyle(() => {
    return {
      borderWidth: radioState.value
        ? withTiming(15)
        : withDelay(300, withTiming(2)),
      borderColor: interpolateColor(radioState.value, [0, 1], colors),
    };
  });
  const firstAnimate = useAnimatedStyle(() => {
    return {
      transform: [
        {
          translateY: radioState.value
            ? withDelay(400, withTiming(0))
            : withDelay(100, withTiming(-30)),
        },
      ],
    };
  });
  const secondAnimate = useAnimatedStyle(() => {
    return {
      transform: [
        {
          translateY: radioState.value
            ? withDelay(450, withTiming(0))
            : withDelay(150, withTiming(-30)),
        },
      ],
    };
  });
  const thirdAnimate = useAnimatedStyle(() => {
    return {
      transform: [
        {
          translateY: radioState.value
            ? withDelay(485, withTiming(0))
            : withDelay(185, withTiming(-30)),
        },
      ],
    };
  });
