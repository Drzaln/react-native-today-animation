...
const LiquidSwitch = ({
  ...
  const firstAnimate = useAnimatedStyle(() => {
    return {
      transform: [
        {
          translateX: switchState.value
            ? withDelay(0, withTiming(25))
            : withDelay(0, withTiming(0)),
        },
      ],
    };
  });
  const secondAnimate = useAnimatedStyle(() => {
    return {
      transform: [
        {
          translateX: switchState.value
            ? withDelay(90, withTiming(33))
            : withDelay(90, withTiming(8)),
        },
      ],
    };
  });
  const thirdAnimate = useAnimatedStyle(() => {
    return {
      transform: [
        {
          translateX: switchState.value
            ? withDelay(145, withTiming(36))
            : withDelay(145, withTiming(11)),
        },
      ],
    };
  });
  ...
  const switchHandler = () => {
    switchState.value = withTiming(!switchState.value, {duration: 200});
  };
