...
const DownloadButton = () => {
    ...
    return {
      width:
        width.value === 1
          ? withTiming(
              '100%',
              {easing: Easing.cubic, duration: 1200},
              finished => {
                if (finished) {
                  success.value = true;
                }
  ...
  const animateText = useAnimatedStyle(() => {
    return {
      opacity:
        width.value > 0.5 ? withTiming(0, {duration: 700}) : withTiming(1),
    };
  });
      ...
      transform: [
        {
          translateY:
            width.value > 0.5
              ? withTiming(100, {duration: 800, easing: Easing.circle})
              : withTiming(0),
        },
      ],
      opacity:
  ...
  return (
    <Pressable style={styles.button} onPress={() => (width.value = 1)}>
      ...
      <View style={styles.downloadContainer}>
        ...
        <Animated.Text style={[styles.text, animateText]}>
          Download
        </Animated.Text>
      </View>
    </Pressable>
